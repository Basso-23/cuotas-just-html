class Myslider {
  constructor(selector, settings) {
    ...
    this.id_slider = this.$el.dataset.mysliderContainer;
    this.$slider = this.$el.querySelector(
      `[data-myslider-slider='${this.id_slider}']`
    );
    this.$next = document.querySelector(
      `[data-myslider-next='${this.id_slider}']`
    );
    this.$prev = document.querySelector(
      `[data-myslider-prev='${this.id_slider}']`
    );
    this.$dots = document.querySelector(
      `[data-myslider-dots='${this.id_slider}']`
    );
    this.dotsItems = null;
    this.activeId = 0;
    this.slideWIdth = this.$el.offsetWidth / this.slidesVisible;
    this.slides = this.$slider.querySelectorAll(
      `[data-myslider-slide='${this.id_slider}']`
    );
  ...
  dotsInit() {
    for (let i = 0; i < this.sectionCount; i++) {
      this.$dots.insertAdjacentHTML(
        ...
        }" data-myslider-dotid='${this.id_slider}'></div>`
      );
    }
    const dots = document.querySelectorAll(
      `[data-myslider-dotid='${this.id_slider}']`
    );
  ...
  activateDot(dots, id) {
    const activeDot = document.querySelector(
      `[data-mysliderdot="${id}"][data-myslider-dotid='${this.id_slider}']`
    );
    if (activeDot) {
      dots.forEach((dot) => {
...
const slider1 = new Myslider("[data-myslider-container='id_slider']", {
  slides: 3,
  responsive: [
    {
      width: 1385,
